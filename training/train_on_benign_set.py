...
def get_checkpoint_path(args):
    ...
    if not os.path.exists(checkpoint_save_path): tools.create_missing_folders(checkpoint_save_path)
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epochs+1):  # train backdoored base model
        ...
        if epoch >= args.checkpoint_save_epoch:
            torch.save(model.module.state_dict(), checkpoint_save_path + '/epoch_' + str(epoch) + '.pt')
